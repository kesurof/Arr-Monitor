...
install_systemd() {
    ...
    if [[ -f "$SCRIPT_DIR/install-arr.sh" ]]; then
        ...
        warn "Script d'installation systemd non trouvé"
        echo ""
        echo "🔧 Configuration manuelle du service systemd:"
        echo ""
        echo "1. Créer le fichier service:"
        ...
        echo "ExecStart=$SCRIPT_DIR/venv/bin/python $SCRIPT_DIR/arr-monitor.py --config $SCRIPT_DIR/config/config.yaml"
        echo "Restart=always"
        echo "RestartSec=10"
        echo ""
        echo "[Install]"
